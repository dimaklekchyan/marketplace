...
internal fun CorChainDsl<QuizQuestionContext>.stubValidationBadGameId(title: String) = worker {
    this.title = title
    on { stubCase == QuizQuestionStubs.BAD_GAME_ID && state == QuizQuestionState.RUNNING }
    handle {
        state = QuizQuestionState.FAILING
        this.errors.add(
            QuizError(
                group = "validation",
                code = "validation-game-id",
                field = "gameId",
                message = "Wrong gameId field"
            )
        )
    }
}