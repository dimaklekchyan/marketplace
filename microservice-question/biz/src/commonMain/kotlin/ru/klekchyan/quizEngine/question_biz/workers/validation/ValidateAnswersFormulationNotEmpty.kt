...
fun CorChainDsl<QuizQuestionContext>.validateAnswersFormulationNotEmpty(title: String) = worker {
    this.title = title
    on {
        questionValidating.type == QuizQuestionType.MULTIPLE_OPTIONS_QUESTION &&
        questionValidating.answers.any { it.formulation.isEmpty() }
    }
    handle {
        fail(
            errorValidation(
                field = "answer formulation",
                violationCode = "empty",
                description = "answer formulation must not be empty"
            )
        )
    }
}