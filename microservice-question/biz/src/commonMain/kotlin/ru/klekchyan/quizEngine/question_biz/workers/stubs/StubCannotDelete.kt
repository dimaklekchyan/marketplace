...
internal fun CorChainDsl<QuizQuestionContext>.stubCannotDelete(title: String) = worker {
    this.title = title
    on { stubCase == QuizQuestionStubs.CANNOT_DELETE && state == QuizQuestionState.RUNNING }
    handle {
        state = QuizQuestionState.FAILING
        this.errors.add(
            QuizError(
                group = "internal",
                code = "internal-cannot-delete",
