...
fun CorChainDsl<QuizQuestionContext>.validateSelectorGameIdProperFormat(title: String) = worker {
    this.title = title
    on {
        questionsSelectorValidating.gameId != QuizCommonEntityId.NONE &&
        !questionsSelectorValidating.gameId.asString().matches(QuizCommonEntityId.formatRegex)
    }
    handle {
        fail(
            errorValidation(
